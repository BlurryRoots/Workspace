...
when you need CORS or rewrite rules. Also allows basic file post handling.
...
class Corsica(SimpleHTTPRequestHandlerStream):
    ...
    def store_file (self, upload_dir, f):
        if isinstance (f, cgi.FieldStorage):
            _print (f'Processing FiledStorage ...')

            file_name = os.path.join (upload_dir, f.filename)
    ...
    def do_POST(self):
        ...
        try:
            ...
            stored_files = []
            _print ('Deciding on how to receive upload ...')
            if isinstance (form['file'], list):
                for f in form['file']:
                    _print (f"File: {type (f)}")
                    file_name = self.store_file (upload_dir, f)
                    if file_name is None:
                        raise Exception (f'Could not store file! {f}')
                    stored_files.append (file_name)
            else:
                raise Exception ('Could not figure out file contents :(')

            k['time'] = f'{rfc8601 ()}'
            k['r'] = 200
            k['filename'] = f"{stored_files}"
